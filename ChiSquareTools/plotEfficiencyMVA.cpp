...
void plotEfficiency(TFile* f) {
  ...
  TH1* eff_jets = (TH1*) f->Get("mva_efficiency_vs_mtt");
  ...
  TLegend* l = new TLegend(0.4, 0.15, 0.85, height + 0.15);
  l->SetTextFont(42);
  l->SetBorderSize(0);
  l->SetFillColor(kWhite);
  l->AddEntry(g_eff_nobtag, "#chi^{2} sorting algorithm", "P");
  l->AddEntry(g_eff_jets, "MVA sorting algortihm (NN)", "P");
  l->Draw();

  c1->Print("mva_selection_efficiency.pdf");
}
