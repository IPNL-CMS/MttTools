...
template<>
void createBranch(TTree* t, const std::string name, int& ptr) {
  t->Branch(name.c_str(), &ptr, (name + "/I").c_str());
}
...
TChain* loadChain(const std::vector<std::string>& inputFiles, const std::string& name) {
  ...
  chain->SetCacheSize(300*1024*1024);
...
void createTree(const std::vector<std::string>& inputFiles, const std::string& outputFile, int64_t maxEntries, float weight, bool isSignal, bool isSemiMu, int btag, bool mva, bool chi2, bool kf, bool hybrid) {
  ...
  createBranch(tree, "n_btagged_jets", n_btagged_jet);
