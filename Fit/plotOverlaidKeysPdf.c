{
  ...
  for (int i = 0; i < n->GetVal(); i++) {
    TString mu = TString::Format("signal_muon_%d", i);
    TString e = TString::Format("signal_electron_%d", i);
...
//    _mu_pdf->plotOn(plot);
  ...
  RooAbsPdf* mu_good_pdf = static_cast<RooAbsPdf*>(w->pdf("old_signal_muon"));
