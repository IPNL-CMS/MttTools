...
void process(int mass, bool muonsOnly, int btag, const std::string& file, bool singleFile, const std::string& signalDatasetFile, __attribute__((unused)) bool fixBackground) {
  ...
  */
  ...
  for (int i = 0; i < NUM_ITER; i++) {
    ...
    TString outputCombineWorkspaceFile = TString::Format("%s/combine/%s_%d_workspace_%d.root", base_path.c_str(), getAnalysisPrefix(), mass, i);
    ...
    if (child == 0) {
      ...
      if (execl("./fitMtt", "fitMtt", "-m", massStr.c_str(), parameter.c_str(), file.c_str(), "--b-tag", btagStr.c_str(), "-o", outputCombineWorkspaceFile.Data(), "--workspace", workspaceFile.Data(), "--combine", NULL) < 0) {
