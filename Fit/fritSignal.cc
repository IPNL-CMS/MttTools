...
void fritSignal(const std::string& file, const std::string& jecType, const std::string& configFile, int massZprime, int btag) {
  ...
  std::map<std::string, std::shared_ptr<BaseFunction>> backgroundPdfs = getCategoriesPdf("./fit_configuration", configFile, Mtt_KF_reco, NULL, massZprime, "background", whichLepton, nullptr);

  // It takes some time, inform the user of what's going on
  std::cout << "Loading signal pdfs... (may takes some time)" << std::endl;
  std::map<std::string, std::shared_ptr<BaseFunction>> signalPdfs = getCategoriesPdf("./fit_configuration", configFile, Mtt_KF_reco, dataset, massZprime, "signal", whichLepton, &configFileId);
  ...
  drawHistograms(whichLepton, Mtt_KF_reco, nBins, *dataset, simPdf, backgroundPdfs, btag, std::string("frit/" + prefix), false);
  drawHistograms(whichLepton, Mtt_KF_reco, nBins, *dataset, simPdf, backgroundPdfs, btag, std::string("frit/" + prefix), true);
  ...
  while ((type = static_cast<RooCatType*>(it->Next()))) {
    ...
    RooDataHist* binnedDataset = new RooDataHist("binnedDataset", "binned dataset", RooArgSet(Mtt_KF_reco), *dataset->reduce(Cut(cut.str().c_str())));
